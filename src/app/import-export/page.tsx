...
import { downloadCsv, parseExecutionHistoryCsv, parseOutletCsv, plannerToCsv } from "@/lib/csv";
...
export default function ImportExportPage() {
  ...
  const plan = useMemo(() => generateMonthlyRoutePlan(new Date().getMonth() + 1, new Date().getFullYear(), outlets, clusters), [outlets]);
  ...
  function mergeExecutionRecords(nextRecords: RouteExecutionRecord[]) {
    ...
    const existing = raw ? (JSON.parse(raw) as RouteExecutionRecord[]) : [];
    const byVisit = new Map(existing.map((record) => [record.visitId, record]));
    nextRecords.forEach((record) => byVisit.set(record.visitId, record));
    window.localStorage.setItem(EXECUTION_STORAGE_KEY, JSON.stringify([...byVisit.values()]));
  }
  ...
  function onHistoryFileChange(file?: File) {
    ...
    file.text().then((content) => {
      ...
      mergeExecutionRecords(parsed.records);
      setHistoryMessage(`Import thành công ${parsed.records.length} dòng lịch sử thực hiện. Vào Planner và chọn tháng sau để xem tuyến bù.`);
    });
  }
  ...
  return (
    <div>
      ...
      <div className="mb-4 grid gap-4 rounded-lg border border-line bg-white p-4 shadow-soft md:grid-cols-3">
        <label className="block">
          <span className="mb-2 block text-sm font-bold">Import lịch sử thực hiện tháng trước</span>
